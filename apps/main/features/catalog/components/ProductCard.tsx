...
export default function ProductCard({ id, title, basePrice, category, totalStock }: ProductCardProps) {
    ...
    return (
        ...
        >
            <Link
                href={`/products/${id}`}
                className="group block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-[2rem]"
                aria-label={`View ${title} — ${formatRsCompact(basePrice)}`}
            >
                <article className="bg-bg-card rounded-[2rem] overflow-hidden border border-border/60 shadow-md hover:shadow-card-hover hover:-translate-y-2 hover:border-primary/20 transition-all duration-500 ease-out h-full flex flex-col isolate">
                    ...
                    {/* Content */}
                    <div className="p-6 sm:p-7 flex flex-col flex-1 bg-bg-card">
                        ...
                        <div className="flex items-center justify-between mt-auto pt-3">
                            ...
                            {/* h-14 min-touch target for mobile users via p flex layout */}
                            <div className="flex items-center justify-center min-h-[56px] min-w-[56px] text-sm text-primary-foreground font-semibold bg-accent px-5 py-2 rounded-full border border-transparent group-hover:bg-primary group-hover:shadow-lg transition-all duration-500 touch-auto">
                                View
                            </div>
                        </div>
                    </div>
